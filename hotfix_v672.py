...
if 'APP_VERSION = "6.7.2"' in src:
    print("[NOVA V6.7.2] Launch hotfix already applied.")
    raise SystemExit(0)

if 'APP_VERSION = "6.7.0"' not in src:
    raise SystemExit("[NOVA V6.7.2] Expected V6.7.0 source after final_v670.py.")

src = src.replace('APP_VERSION = "6.7.0"', 'APP_VERSION = "6.7.2"', 1)
...
if runtime_anchor not in src:
    raise SystemExit("[NOVA V6.7.2] Runtime telemetry anchor not found.")
        ...
        while True:
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    seeds=seeds[:cap]
                    if seeds and _trade_subscription_allowed():
                    ...
                    async for raw in ws:
                        ...
                        if is_create:
                            ...
                            cap=max(8,i("pulse_max_trade_subscriptions")) if FREE_LITE else max(30,i("pulse_max_trade_subscriptions"))
                            if mint not in runtime["pulse_subscribed"] and _trade_subscription_allowed():
...
if n != 1:
    raise SystemExit("[NOVA V6.7.2] PumpPortal loop block not found.")
...
if dash_anchor not in src:
    raise SystemExit("[NOVA V6.7.2] Dashboard realtime anchor not found.")
...
if diag_anchor not in src:
    raise SystemExit("[NOVA V6.7.2] Diagnostic endpoint anchor not found.")
src = src.replace(diag_anchor, diag_replacement, 1)

src = src.replace("/6.7.0", "/6.7.2")
compile(src, str(APP_PATH), "exec")
APP_PATH.write_text(src, encoding="utf-8")
print("[NOVA V6.7.2] Metered trade-stream fix applied and syntax-validated.")
